...
params = {}
#训练集
params['trainpath'] = '../dataset/traindata_N15_M01_F10.csv'
params['trainopath'] = '../datafeature/traindata_N15_M01_F10_feature_select.csv'

#测试集
params['testpath'] = '../dataset/traindata_N15_M07_F04.csv'
params['testopath'] = '../datafeature/traindata_N15_M07_F04_feature_select.csv'
...
try:
    ...
    #训练集1特征提取
    file_path = params['trainpath']
    features = feature_get(file_path)
    col_lab = columns_list_all + ['label']
    result = pd.DataFrame(features, columns = col_lab)
    result.to_csv(params['trainopath'], sep=',', header=True, index=False)
    
    #训练集2特征提取
    file_path = params['testpath']
    features = feature_get(file_path)
    col_lab = columns_list_all + ['label']
    result = pd.DataFrame(features, columns = col_lab)
    result.to_csv(params['testopath'], sep=',', header=True, index=False)   
